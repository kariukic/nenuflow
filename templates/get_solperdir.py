...
parser.add_argument('--solint', help='Solint parameter in DP3', type=int, required=False, default=30)
parser.add_argument('--max_flux', help='Maximum flux in a single solution', type=float, required=False, default=50.0)
parser.add_argument('--min_flux', help='Minimum flux in a single solution', type=float, required=False, default=30.0)
parser.add_argument('--dist', help='Include angular distance of patch in solint calculation', type=str2bool, required=False, default=False)
...
#     separations = sm.getDistance(main['Main'][0].value,main['Main'][1].value,byPatch=True)
...
def get_flux(flux, alpha, logSI, nterms, ref_freq, target_freq):
    if nterms > 1:
        for i in range(nterms):
            if logSI == 'true':
                flux *= 10.0**(alpha[i] * (np.log10(target_freq/ref_freq))**(i+1))
            else:
...
def get_solints(skymodel, solint, max_flux, min_flux, dist=False):
    patches, patch_fluxes, separations = get_patch_fluxes(skymodel)
    main_idx = patches.index('Main')
    patch_fluxes.pop(main_idx)
    flux_norm = np.array(patch_fluxes)/np.maximum(np.minimum(max_flux, min(patch_fluxes)), min_flux)
    if dist:
...
def main():
    get_solints(args.skymodel, solint=args.solint, max_flux=args.max_flux, min_flux=args.min_flux, dist=args.dist)
