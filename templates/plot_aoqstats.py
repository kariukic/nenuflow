...
if __name__ == "__main__":
    ...
    aoq = ao.AOQualityTimeStat.from_ms_list([args.qs_file])
    stat_data = aoq.get_combined_stat(args.stat)
    output_data = np.zeros((np.shape(stat_data[0])[0], 3), dtype="complex")
    output_data[:, 0] = stat_data[0]
    output_data[:, 1] = stat_data[2][0, :, 0]
    output_data[:, 2] = stat_data[2][0, :, 3]
